...
class ContractApiTest(unittest.TestCase):
    ...
    def test_get_contract_by_requester(self):
        ...
        r = requests.get(ContractApiTest.CONTRACT_URL +
                         "query/contractbyrequester/{}/".format(requester))
        self.assertEqual(r.status_code, 200)
    ...
    def test_get_contract_by_provider(self):
        provider = 'Brade'
        r = requests.get(ContractApiTest.CONTRACT_URL +
                         "query/contractbyrequester/{}/".format(provider))
        self.assertEqual(r.status_code, 200)
    ...
    def test_get_contract_by_id(self):
        id = 'kg244565'
        r = requests.get(ContractApiTest.CONTRACT_URL +
                         "query/contractbyrequester/{}/".format(id))
        self.assertEqual(r.status_code, 200)
    # new contract

    def test_new_contract(self):
        r = requests.post(ContractApiTest.CONTRACT_URL +
                          "/contract/create/", json=ContractApiTest.DATA)
        self.assertEqual(r.status_code, 200)
