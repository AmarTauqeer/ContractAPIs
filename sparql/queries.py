...
class SPARQL(UserCredentials):
    ...
    def get_all_contracts(self):
        sparql = self.init_sparql(
            self.HOST_URI_GET, self.get_username(), self.get_password())
        ...
        if response:
            ...
            res = jsonify({'Error': 'Something wrogn with contracts data'})
            res.status_code = 403
            return res
    ...
    def get_contract_by_requester(self, name):
        ...
        if response['results']['bindings'] != []:
            ...
            res = jsonify(
                {'Not Found': 'No contract with this contract requester'})
            res.status_code = 404
            return res
    ...
    def get_contract_by_provider(self, name):
        ...
        if response['results']['bindings'] != []:
            ...
            res = jsonify(
                {'Not Found': 'No contract with this contract provider'})
            res.status_code = 404
            return res
    ...
    def get_contract_by_id(self, id):
        ...
        if response['results']['bindings'] != []:
            ...
            res = jsonify(
                {'Not Found': 'No contract with this contract id'})
            res.status_code = 404
            return res
    ...
    def contract_update_by_id(self, id):
        ...
        if response['results']['bindings'] != []:
            ...
            res = jsonify(
                {'Not modified': 'Contract not midified'})
            res.status_code = 304
            return res
